...
open class CurveChartView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    View(context, attrs) {
    ...
    init {
        val typedArray = context.obtainStyledAttributes(
            ...
            R.attr.curveChartView,
            R.style.CurveChartView
        )
        ...
        setLabelTextSize(
            typedArray.getDimensionPixelSize(
                R.styleable.CurveChartView_cv_labelTextSize,
                0
            )
        )
    ...
    private fun drawScaleLabel(canvas: Canvas) {
        ...
        for (i in 0 until this.config.partCount) {
            ...
            val label = String.format(
                "%.1fM",
                (this.yMaxValue - this.yMinValue) * i * 1f / (config.partCount - 1) + this.yMinValue
            )
    ...
    private fun drawLine(canvas: Canvas, size: Int) {
        ...
        this.linePath.moveTo(
            paddingLeft.toFloat(),
            height - paddingBottom * 1f - (this.dataItem[0] - this.yMinValue) * this.yLenPerValue
        )
        this.fillPath.moveTo(paddingLeft.toFloat(), height - paddingBottom * 1f)
        this.fillPath.lineTo(
            paddingLeft.toFloat(),
            height - paddingBottom * 1f - (this.dataItem[0] - this.yMinValue) * this.yLenPerValue
        )
        ...
        for (i in 1 until size) {
            ...
            this.linePath.lineTo(
                paddingLeft.toFloat() + i.toFloat() * this.xLenPerCount,
                height - paddingBottom * 1f - (value - this.yMinValue) * this.yLenPerValue
            )
            this.fillPath.lineTo(
                paddingLeft.toFloat() + i.toFloat() * this.xLenPerCount,
                height - paddingBottom * 1f - (value - this.yMinValue) * this.yLenPerValue
        ...
        this.fillPath.lineTo(
            paddingLeft.toFloat() + (size - 1).toFloat() * this.xLenPerCount,
            height - paddingBottom * 1f
        )
        this.fillPath.close()
