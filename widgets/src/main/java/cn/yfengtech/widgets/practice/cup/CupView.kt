package cn.yfengtech.widgets.practice.cup
...
class CupView @JvmOverloads constructor(
    context: Context,
    ...
    val waterPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        ...
        shader = LinearGradient(
            0f,
            waterRect.top.toFloat(),
            0f,
            waterRect.bottom.toFloat(),
            Color.parseColor("#4486ed"),
            Color.parseColor("#363c48"),
    ...
    var xRange = IntRange(
        (waterRect.left + waterRect.right) / 2 - 2,
        (waterRect.left + waterRect.right) / 2 + 2
    )
    var yRange =
        IntRange(
            (cupHeight / 2 - cupWallWidth / 2).toInt() - 2,
            (cupHeight / 2 - cupWallWidth / 2).toInt() + 2
        )
